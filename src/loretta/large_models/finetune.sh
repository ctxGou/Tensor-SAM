...
python run.py \
    --model_name $MODEL \
    --task_name $TASK \
    --output_dir ./result/$TASK-${MODEL_NAME}-$RANK-$TAG-$TRAINER --tag $TAG --tuning_type $TYPE --train_set_seed $SEED --num_train $TRAIN --num_dev $DEV --num_eval $EVAL --logging_steps 10 \
