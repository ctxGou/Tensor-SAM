...
from mytrainer import SAMTrainer, BARTrainer
...
class OurArguments(TrainingArguments):
    ...
    # [cao]: SAM and gBAR
    sam_rho: float = 0 # rho for SAM
    gbar_alpha: float = 0 # alpha for gBAR
...
class Framework:
    ...
    def load_model(self):
        ...
        if self.args.tuning_type == 'prompt':
            ...
            from peft_local import get_peft_model, PromptTuningInit, PromptTuningConfig, TaskType
        ...
        if self.args.tuning_type == 'ia3':
            # from prefix import PrefixTuning
            # PrefixTuning(model, num_prefix=5, reparam=False, float16=False, init_by_real_act=True)
            from peft_local import get_peft_model, IA3Config, TaskType
    ...
    def train(self, train_samples, eval_samples):
        ...
        elif self.args.trainer == 'sam':
            trainer_class = SAMTrainer
        elif self.args.trainer == 'gbar2':
            trainer_class = BARTrainer
            ...
            for train_set_id, train_samples in enumerate(train_sets):
                ...
                if not args.no_eval:
                    ...
                    if args.local_rank <= 0:
                        write_metrics_to_file(metrics, "result/" + result_file_tag(args) + f"-trainset{train_set_id}-rank-{args.rank}-{args.trainer}-{args.sam_rho}-{args.gbar_alpha}" + ".json" if args.result_file is None else args.result_file)
        else:
            if args.num_eval is not None:
