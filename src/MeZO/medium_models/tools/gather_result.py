...
def main():
    ...
    for tag in all_tags:
        for k in all_k:
            ...
            seed_result = {
                # [cao]: add time here for no duplicates
                # s: list({x['output_dir']: x for x in v}.values())
                s: list({x['output_dir'] + x['time']: x for x in v if x['sam_rho'] in [0.001, 0.01, 0.1] #if x['sam_rho'] in [0.01, 0.02]
                         }.values())
            ...
            if len(final_result_test) > 0:
                s += "%.2f (%.2f) (#seeds %s) (#runs %s) (median %.2f)" % (final_result_test.mean() * 100, final_result_test.std() * 100, len(final_result_test), num_results.sum(), np.median(final_result_test) * 100,)
                if len(args.test_key2) > 0:
