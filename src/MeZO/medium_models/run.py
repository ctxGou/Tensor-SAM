...
class ModelArguments:
    ...
    # [cao]: Apply Flora (ICLR 2025). Only works if apply_lora is True.
    apply_flora: bool = field(
        default=False,
        metadata={'help': 'use Flora for finetuning'}
    )

    # [cao]: Apply BAR.
    apply_bar: bool = field(
        default=False,
        metadata={'help': 'use BAR for finetuning'}
    )
...
class DynamicTrainingArguments(TrainingArguments):
    ...
    # sam and gbar arguments
    sam_rho: float = field(
        default=-1,
        metadata={"help": "rho for SAM optimizer"}
    )
    gbar_alpha: float = field(
        default=-1,
        metadata={"help": "alpha for gBAR optimizer"}
    )
    ...
    if training_args.do_train:
        ...
        if training_args.trainer == "standard" or training_args.trainer == "linearhead":
            ...
            if training_args.evaluate_during_training:
                ...
                # deleting "module." prefix in the best_model_ckpt's keys
                trainer.best_model_ckpt = {k.replace("module.", ""): v for k, v in trainer.best_model_ckpt.items()}
    ...
    if trainer.is_world_process_zero():
        with FileLock('log.lock'):
            with open(training_args.log_file, 'a') as f:
                ...
                if 'eval_strategy' in final_result:
                    final_result.pop('eval_strategy')

                # [cao]: a hack for gather_result.py
                final_result['seed'] = int(training_args.seed)
                final_result['learning_rate'] = float(training_args.learning_rate)
                final_result['per_device_train_batch_size'] = int(training_args.per_device_train_batch_size)
                final_result['optimizer'] = training_args.optimizer
                final_result['gbar_alpha'] = training_args.gbar_alpha
                final_result['sam_rho'] = training_args.sam_rho
