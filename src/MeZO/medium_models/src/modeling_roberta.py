...
# [cao]: import flora here
from src.tensor_lora.layers_flora import Linear as FloraLinear
import src.tensor_lora.layers_LoRA as lora
...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        if getattr(config, "apply_lora", False):
            if config.flora:
                self.query = FloraLinear(config.hidden_size, self.all_head_size, config.lora_r, lora_alpha=config.lora_alpha)
            else:
        ...
        if getattr(config, "apply_lora", False):
            if config.flora:
                self.value = FloraLinear(config.hidden_size, self.all_head_size, config.lora_r, lora_alpha=config.lora_alpha)
            else:
