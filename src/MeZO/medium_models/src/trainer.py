...
from src.tensor_lora.gbar import gBAR
from src.tensor_lora.sam import SAM
...
class Trainer(LinearHeadTrainer):
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer is None:
            ...
            elif self.args.optimizer == 'sam_sgd':
                self.optimizer = SAM(
                    optimizer_grouped_parameters,
                    base_optimizer=SGD,
                    rho=self.args.sam_rho,
            ...
            elif self.args.optimizer == 'gbar_sgd':
                self.optimizer = gBAR(
                    total_steps=num_training_steps,
                    params=optimizer_grouped_parameters,
                    model=self.model,
        ...
        for epoch in range(epochs_trained, int(num_train_epochs)):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # standard, non-ZO optimization
                    ...
                    if self.args.optimizer in {'sam', 'sam_sgd'}:
                        # SAM optimizer requires second step
                        optimizer.first_step(zero_grad=True)
                        self.training_step(model, inputs)  # second step
                    ...
                    if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                        ...
                        if (self.args.logging_steps > 0 and self.state.global_step % self.args.logging_steps == 0) or (
                            ...
                            # from src.tensor_lora.layers_flora import Linear as FloraLinear
                            # for name, layer in model.named_modules():
                            #     if isinstance(layer, FloraLinear):
                            #         weight_grad_norm_statistics = layer.weight_norm_gradient_norm_statistics()
                            #         # this is a dictionary. add name to the keys
                            #         for k, v in weight_grad_norm_statistics.items():
                            #             logs[f'{name}_{k}'] = v

                            # from src.tensor_lora.layers_LoRA import Linear as LoRALinear
                            # for name, layer in model.named_modules():
